...
[McpServerToolType]
public static class AnalyzeSymbolUsageTool
{
    ...
    [McpServerTool, Description("Analyzes a .NET solution to find usage of a namespace, type, or member. Returns detailed information about where and how the symbol is used in the codebase.")]
    public static async Task<string> AnalyzeSymbolUsage(
        [Description("Path to the .NET solution file (.sln)")]
        string solutionPath,
        [Description("Namespace, type, or member to analyze (e.g. 'System.IO', 'System.Diagnostics.Stopwatch', 'Console.WriteLine')")]
        string targetSymbol,
        ...
        int contextLines = 0)
    {
        ...
        try
        {
            var result = await _analyzer.AnalyzeSymbolAsync(solutionPath, targetSymbol, false, contextLines);
